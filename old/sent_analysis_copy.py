...
def original_final_gpt_sent_identifier(df: pd.DataFrame, index: int, original_gpt_list: list,
    ...
    if revise(df, index):
        bool_index = in_gpt_range(df, index, get_gpt_inti_sent(df[:index]))
        update = bool_index[0]
        sent_index = bool_index[1]
        if update:
            return [True, [original_gpt_list[sent_index], final_gpt_list[sent_index]]]
...
def creative_code_identifier(df: pd.DataFrame, index: int, sentences: list, sent_positions: list) -> set:
    ...
    if sent_index is not None:
        if len(sent_index) != 0:
            ...
            if sent_index == prompt_index + 1:
                creative_seq.add('narrativeProg')
            # 1.2 Scene Transition
            if (token['entity'] == 'B-LOC' or 'I-LOC' or 'B-ORG' or 'I-ORG' for token in sent_NER):
                creative_seq.add('narrativeProg')
            # 1.3 Climax
            if '!' in sentence:
                creative_seq.add('narrativeProg')
            ...
            for token in sent_NER:
                if (token['entity'] == 'B-PER' or 'I-PER') and token['word'] not in entity:
            ...
            for token in pos_tags:
                ...
                # 2.2 Metaphor
                if 'NN' in token and 'is' or 'are' or 'were' or 'was' in sentence:  # TODO
                    creative_seq.add('stylisticDev')
                # 3.3 Mystery & 3.4 Dialogue
                if '``' in token:
                    creative_seq.add('characterization')
            # 2.3 Contrast/turning point
            if 'but' or 'however' or 'whereas' in sentence:
                creative_seq.add('stylisticDev')
            # 2.4 Echo
            model = SentenceTransformer('sentence-transformers/all-MiniLM-L6-v2')
            ...
            if (token['entity'] == 'B-PER' or 'I-PER' for token in
                sent_NER) and 'believe' or 'think' or 'wonder' in sentence:
                creative_seq.add('stylisticDev')
            # case 3: Characterization
            # 3.1 Stake
            if 'have to' or 'must' or 'hope' or 'goal' in sentence:
                creative_seq.add('characterization')
            # 3.2 Reaction
...
for i in range(len(data)):
    in_which_gpt = original_final_gpt_sent_identifier(data, i, original_gpt_list=original_gpt, final_gpt_list=final_gpt)
    code_list = []
...
df2.to_csv('sample_01a3e_creative.csv')
